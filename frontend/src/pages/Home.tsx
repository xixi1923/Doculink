  ...
  return (
    <div className="bg-slate-50/50 dark:bg-gray-950 min-h-screen pb-16 text-slate-800 dark:text-gray-200 font-sans selection:bg-teal-500 selection:text-white transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* ================= BROWSE CATEGORIES MODULE ================= */}
        <div className="mb-14">
          <div className="flex items-end justify-between mb-6 px-1">
            <div>
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Browse by Material Type</h2>
              <p className="text-sm text-slate-400 dark:text-gray-500 font-medium">Categorized resource index grids</p>
            </div>
            <Link to="/search" className="text-sm font-bold text-teal-500 hover:text-teal-600 transition-colors flex items-center gap-0.5 group">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {isLoading ? (
              [...Array(6)].map((_, i) => (
                <div key={i} className="bg-white dark:bg-gray-900 p-5 rounded-2xl shadow-xs border border-slate-100 dark:border-gray-800 animate-pulse min-h-[140px] flex flex-col items-center justify-center">
                  <div className="w-11 h-11 bg-slate-100 dark:bg-gray-800 rounded-xl mb-3" />
                  <div className="h-4 bg-slate-100 dark:bg-gray-800 rounded w-20 mb-2" />
              ...
              categories.map((cat) => {
                ...
                return (
                  <Link
                    ...
                    key={cat.id}
                    className="bg-white dark:bg-gray-900 p-5 rounded-2xl shadow-xs border border-slate-100 dark:border-gray-800 hover:shadow-md dark:hover:shadow-gray-900/50 hover:-translate-y-0.5 transition-all cursor-pointer group text-center flex flex-col items-center justify-center min-h-[140px]"
                  >
                    ...
                    <h3 className="font-bold text-slate-800 dark:text-gray-200 text-sm mb-0.5 truncate w-full">{cat.name}</h3>
                    <p className="text-[10px] text-slate-400 dark:text-gray-500 font-bold uppercase tracking-wide">{cat.documents_count || 0}+ Files</p>
                  </Link>
                )
              })
            )}
        ...
        <div className="mb-14">
          <div className="flex items-end justify-between mb-6 px-1">
            <div className="flex items-center gap-2">
               ...
               <div>
                 ...
                 <p className="text-sm text-slate-400 dark:text-gray-500 font-medium">Curated base entries from your curriculum match</p>
               </div>
            </div>
            <div className="flex gap-1.5">
              <button className="p-1.5 bg-white dark:bg-gray-900 rounded-lg border border-slate-200 dark:border-gray-800 text-slate-400 hover:text-slate-800 dark:hover:text-white transition-colors"><ChevronLeft size={16} /></button>
              <button className="p-1.5 bg-teal-500 rounded-lg text-white hover:bg-teal-600 transition-all"><ChevronRight size={16} /></button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {isLoading ? (
              [...Array(4)].map((_, i) => (
                <div key={i} className="bg-white dark:bg-gray-900 rounded-[24px] overflow-hidden border border-slate-100 dark:border-gray-800 flex flex-col w-full animate-pulse min-h-[400px]">
                  <div className="aspect-[4/3] bg-slate-100 dark:bg-gray-800" />
                  <div className="p-5 flex-grow space-y-4">
                    <div className="h-6 bg-slate-100 dark:bg-gray-800 rounded w-3/4" />
                    <div className="h-4 bg-slate-100 dark:bg-gray-800 rounded w-1/2" />
                    <div className="pt-4 border-t border-slate-100 dark:border-gray-800 flex justify-between">
                      <div className="w-6 h-6 bg-slate-100 dark:bg-gray-800 rounded-full" />
                      <div className="w-20 h-4 bg-slate-100 dark:bg-gray-800 rounded" />
                    </div>
                  </div>
                </div>
              ))
              ...
              documents.slice(0, 4).map((doc) => (
                <Link to={`/documents/${doc.id}`} key={doc.id} className="group flex">
                  <div className="bg-white dark:bg-gray-900 rounded-[24px] overflow-hidden border border-slate-100 dark:border-gray-800 flex flex-col w-full hover:shadow-xl dark:hover:shadow-teal-500/10 hover:-translate-y-1 transition-all duration-300 min-h-[400px]">

                    {/* Aspect Document Face Frame */}
                    <div className="relative aspect-[4/3] overflow-hidden bg-slate-100 dark:bg-gray-800">
                      ...
                      <div className="absolute top-3 left-3 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md px-2.5 py-0.5 rounded-md text-[10px] font-bold text-teal-600 dark:text-teal-400 uppercase tracking-wider shadow-xs">
                      ...
                      <div className="absolute bottom-3 right-3 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                         <div className="w-8 h-8 bg-teal-500 text-white rounded-lg flex items-center justify-center shadow-md hover:bg-teal-600"><Download size={14} /></div>
                      </div>
                    </div>

                    {/* Body Copy Blocks */}
                    <div className="p-5 flex flex-col flex-grow justify-between">
                      <h3 className="font-bold text-lg text-slate-900 dark:text-white line-clamp-2 leading-tight group-hover:text-teal-500 transition-colors">
                        {doc.title}
                      </h3>

                      <div className="pt-4 border-t border-slate-100 dark:border-gray-800 flex items-center justify-between mt-4">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <div className="w-6 h-6 bg-slate-100 dark:bg-gray-800 rounded-full flex items-center justify-center text-[11px] font-bold text-slate-500 dark:text-gray-400 shrink-0">
                             {doc.user?.name?.charAt(0) || 'U'}
                          </div>
                          <span className="text-xs font-bold text-slate-500 dark:text-gray-400 truncate">{doc.user?.name || 'Anonymous'}</span>
                        </div>
                        <div className="flex items-center gap-2.5 text-[10px] font-bold text-slate-400 dark:text-gray-500 uppercase tracking-wide shrink-0">
